...
@app.route('/claimWin', methods=['POST'])
@cross_origin()
def claim_win():
    global USERS, GAMES
    jsdata = request.data
...
@app.route('/randomgame', methods = ['POST'])
@cross_origin()
def get_post_rand():
    global GAMES, USERS
    tx_hash_i = None
...
@app.route('/moverand', methods = ['POST'])
@cross_origin()
...
def get_post_user():
    ...
    resp.set_cookie("login", b'1', max_age=60 * 30, samesite='Lax')
    resp.set_cookie("user", u.encode(), max_age=60 * 30, samesite='Lax')
    resp.set_cookie("addr", acc, max_age=60 * 30, samesite='Lax')
